...
let nimbus_screen = {
        ...
        viewFilePreview: function () {
            ...
            let $nsc_preview_content = $('#nsc_preview_content');
            // let $nsc_stream_video = $('#nsc_stream_video');
            let $nsc_preview_img = $('#nsc_preview_img');
            let $nsc_indicator = $('#nsc_indicator');
            let width = 400;

            if (nimbus_screen.info.file.format === 'webm' || nimbus_screen.info.file.format === 'mp4') {
                let $video = $('<video>').attr({
                    'src': nimbus_screen.info.file.patch + '?' + Date.now(),
                    'preload': '',
                    'controls': '',
                    'autoplay': '',
                ...
                $video[0].oncanplay = function () {
                    if ($video.width() > width) width = $video.width();
                    $nsc_indicator.css({'max-width': width});
                    nimbus_screen.dom.$nsc_indicator.show();
                    nimbus_screen.dom.$preview_loading.hide();
                };
                $nsc_preview_content.find('video').remove();
                $nsc_preview_content.append($video);
            } else {
                $nsc_preview_img.show().attr('src', nimbus_screen.info.file.patch).on("load", function () {
        ...
        //         watermark.src = localStorage.fileWatermark;
        //     } else {
        //         const font = nimbus_screen.sizeFont({
        ...
        initVideoPage: function () {
            nacl_module.blobToFile(nimbus_screen.editVideoInit);
        },
